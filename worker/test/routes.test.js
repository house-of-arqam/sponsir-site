...
const ORIGIN = 'https://sponsir.app';

function req(path, { method = 'POST', body, ip = '1.2.3.4', origin = ORIGIN, headers = {} } = {}) {
  return new Request(`https://api.sponsir.app${path}`, {
    method,
...
test('POST /waitlist rejects invalid email and bad JSON', async () => {
  ...
  res = await worker.fetch(new Request('https://api.sponsir.app/waitlist', { method: 'POST', body: '{oops' }), env, ctx());
  assert.equal(res.status, 400);
});
