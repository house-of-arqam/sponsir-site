# LegitPitch — site, free Pitch Checker and waitlist

Week-1 validation package for **LegitPitch**, a Gmail extension for creators that
...
1. Register `legitpitch.app` (see SPEC.md §Name; re-check RDAP + trademark first) and
...
3. GitHub → Settings → Pages → Source = GitHub Actions. `docs/CNAME` is already
   `legitpitch.app`. Pushing to `main` deploys via `.github/workflows/pages.yml`.
4. Read the gate numbers with `curl -H "Authorization: Bearer $ADMIN_TOKEN" https://api.legitpitch.app/stats`.

Until the Worker is live, set `waitlistEndpoint: ''` in `docs/assets/site-config.js`
and the form falls back to a `mailto:` link.
