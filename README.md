# Sponsir — site, free Pitch Checker and waitlist

Week-1 validation package for **Sponsir**, a Gmail extension for creators that
...
1. Register `sponsir.app` (see SPEC.md §Name; re-check RDAP + trademark first) and
...
3. GitHub → Settings → Pages → Source = GitHub Actions. `docs/CNAME` is already
   `sponsir.app`. Pushing to `main` deploys via `.github/workflows/pages.yml`.
4. Read the gate numbers with `curl -H "Authorization: Bearer $ADMIN_TOKEN" https://api.sponsir.app/stats`.
...
`https://api.sponsir.app/waitlist` and `https://api.sponsir.app/event`.
