(function () {
  ...
  const engine = window.LegitPitchChecker;
...
Maya`
  ...
  if (clearButton) {
    clearButton.addEventListener('click', () => {
      input.value = '';
      output.innerHTML = '<p class="result-empty">Paste a pitch and click <strong>Check this pitch</strong>.</p>';
      input.focus();
    });
  }
  ...
  const VERDICTS = {
    low: {
      ...
      icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><path d="M22 4 12 14.01l-3-3"/></svg>'
    },
    medium: {
  ...
  function render(result) {
    ...
    output.innerHTML = `
      <div class="level-${esc(result.level)}">
        <div class="verdict">
          <div class="verdict-badge">${verdict.icon}</div>
